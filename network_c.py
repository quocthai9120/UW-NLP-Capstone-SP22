...
class ClipCocoDataset(Dataset):
    ...
    def __init__(self, run_type: str, prefix_length: int, gpt2_type: str = "gpt2", clip_model_type: str = "ViT-B/32", device: str = "cuda", normalize_prefix: bool = False, unique: bool = False):
        ...
        if unique:
            ...
            updated_captions_tokens = []
            for i, tup in enumerate(self.caption_data):
                if tup['image_id'] in self.image_ids:
                    continue
                self.image_ids.append(tup['image_id'])
                updated_caption_data.append(tup)
                updated_captions_tokens.append(self.captions_tokens[i])
            self.caption_data = updated_caption_data
            self.captions_tokens = updated_captions_tokens
...
class ClipCaptionModel(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, prefix: torch.Tensor, sequence_embedding: torch.Tensor, mask: Optional[torch.Tensor] = None,
        ...
        out_dict["caption_logits"] = self.gpt(inputs_embeds=embedding_cat, labels=labels, attention_mask=mask)
        return out_dict
